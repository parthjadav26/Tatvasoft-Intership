...
namespace Mission.Services.Service
{
    public class MissionService(IMissionRepository missionRepository) : IMissionService
    {
        ...
        public Task<(bool result, string message)> ApplyMission(ApplyMissionRequestModel model)
        {
            return _missionRepository.ApplyMission(model);
        }

        public Task<List<MissionApplicationResponseModel>> GetMissionApplicationList()
        {
            return _missionRepository.GetMissionApplicationList();
        }

        public Task<bool> MissionApplicationApprove(MissionApplicationResponseModel model)
        {
            return _missionRepository.MissionApplicationApprove(model);
        }

        public Task<bool> MissionApplicationDelete(MissionApplicationResponseModel model)
        {
            return _missionRepository.MissionApplicationDelete(model);
        }
    }
}
